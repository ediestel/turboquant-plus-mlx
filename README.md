...
|--------|-------------|------------|-----|
| TurboQuant 2-bit | 6.4× | 0.79 | 0.0047 |
| TurboQuant 2.5-bit (outlier) | **4.9×** | 0.86 | 0.0029 |
| TurboQuant 3-bit | 4.6× | 0.91 | 0.0018 |
| TurboQuant 3.5-bit (outlier) | **3.8×** | 0.95 | 0.0009 |
| TurboQuant 4-bit | 3.6× | 0.96 | 0.0007 |

> Compression ratios include norm storage overhead (TurboQuant stores 2 × float32 norms per vector; outlier variant stores 3 × float32). These are Python prototype theoretical ratios — the C implementation uses efficient bit packing and may differ.
...
### MLX Usage (Apple Silicon — GPU-Accelerated)

**Requirements:** Apple Silicon Mac (M1–M5) running **macOS 14.0 Sonoma or later**. MLX 0.22+ does not support older macOS versions. If you are on macOS 13 Ventura with an M1/M2, pin an older release:

```bash
pip install "mlx>=0.16.0,<0.22.0"  # macOS 13 Ventura fallback
```

Install the MLX optional dependencies:
...
**Run parity tests** (verifies MLX matches NumPy within tolerance):

```bash
pytest tests/test_mlx/ -v
```

**Expected performance on Apple Silicon (M3 Max estimates):**

| Operation | NumPy (CPU) | MLX (GPU) | Speedup |
|-----------|-------------|-----------|---------|
| Dense rotation (d=128) | ~50 μs | ~5 μs | ~10× |
| Fast WHT (d=128) | ~20 μs | ~3 μs | ~7× |
| Full compress (32 heads, 4K seq) | ~8 ms | ~0.5 ms | ~16× |
| Full decompress (same) | ~5 ms | ~0.3 ms | ~17× |

MLX uses unified memory — zero-copy between CPU and GPU, no explicit device transfers needed.
...
| Model | Context | KV cache | Fits? |
|-------|---------|---------|-------|
| 7B (Q4_K_M weights ~4GB) | 32K | ~1.8GB → ~0.4GB turbo3 | Yes, easily |
| 7B (Q4_K_M) | 128K | ~7GB → ~1.5GB turbo3 | Yes |
| 14B (Q4_K_M ~8GB) | 32K | ~3.5GB → ~0.75GB turbo3 | Yes, but tight uncompressed — use turbo |
| 14B (Q4_K_M) | 64K | ~7GB → ~1.5GB turbo3 | Yes with turbo — no without |
| 32B (Q4_K_M ~18GB) | 8K | ~3.5GB → ~0.75GB turbo3 | Tight but yes |
| 32B (Q4_K_M) | 32K | ~14GB → ~3GB turbo3 | Yes with turbo3 — no without |

**The jump turbo3 enables on 16GB:** Without compression, a 14B model at Q4_K_M leaves ~8GB for KV cache — about 36K context. With turbo3 that same 8GB stretches to ~170K context. You effectively get long-context capability that would otherwise need 64GB hardware.

**Recommended starting config for 16GB:**
...
Asymmetric `q8_0-K + turbo4-V` is the safe default — validated on M1 hardware, no quality regression on most models.

> **Honest caveat:** 32B models are a squeeze on 16GB regardless. You get them loaded but decode will be slow (~3–5 tok/s) because weights alone eat most of the RAM.
...
├── mlx/               # MLX-native GPU implementations (Apple Silicon)
...
├── test_mlx/          # MLX parity tests (run on Apple Silicon with mlx installed)
                       # The full turboquant/ package (including original NumPy files) is
                       # retained so these tests can verify MLX output matches the audited
                       # NumPy reference after each improvement.
benchmarks/
...
| MLX port | ✅ | GPU-accelerated MLX backend for Apple Silicon — `turboquant/mlx/` + `turboquant/integrations/mlx_lm.py` |
