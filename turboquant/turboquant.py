...
class CompressedVector:
    ...
    Stores 2 float32 norms per vector:
      vector_norms   — original ||x||_2, used to rescale PolarQuant reconstruction
      residual_norms — ||residual||_2, used to scale QJL reconstruction
    Memory per vector: d * bit_width bits (indices + signs) + 64 bits (2 × float32 norms).
    """
...
class TurboQuant:
    ...
    def compressed_size_bits(self, n_vectors: int) -> int:
        ...
        - vector_norms: 32 bits (float32) per vector
        - residual_norms: 32 bits (float32) per vector
        """
        per_vector = self.d * self.bit_width  # (b-1) + 1 bits per coordinate
        norms = 64  # 2 × float32: vector_norms + residual_norms
    ...
    def compression_ratio(self, original_bits_per_value: int = 16) -> float:
        ...
        compressed_per_vector = self.d * self.bit_width + 64  # +64: vector_norms + residual_norms
