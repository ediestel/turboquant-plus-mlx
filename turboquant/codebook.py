...
def nearest_centroid_indices(values: np.ndarray, centroids: np.ndarray) -> np.ndarray:
    ...
    boundaries = (centroids[:-1] + centroids[1:]) / 2.0
    # Cast to uint8: max index = 2^bit_width - 1 ≤ 255 for bit_width 1–8 (7–8× memory savings vs int64)
    return np.searchsorted(boundaries, values.ravel()).reshape(values.shape).astype(np.uint8)
