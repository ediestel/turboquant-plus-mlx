...
class OutlierTurboQuant:
    ...
    def compression_ratio(self, original_bits: int = 16) -> float:
        ...
        Norm overhead per vector (3 × float32 = 96 bits):
          - residual_norms (32 bits): QJL residual norm
          - outlier_norms  (32 bits): L2 norm for outlier PolarQuant sub-vector
          - normal_norms   (32 bits): L2 norm for normal PolarQuant sub-vector
        """
        # Effective quantization bits + QJL residual norm
        per_vector_bits = self.d * self.effective_bits + 32  # +32 for residual_norms
        # outlier_norms (32 bits) + normal_norms (32 bits)
        per_vector_bits += 64
