...
class QJL:
    ...
    def __init__(self, d: int, seed: int = 123):
        ...
        # Random projection matrix S ∈ R^(d×d), entries ~ N(0,1); float32 to match sign dtype
    ...
    def dequantize(self, signs: np.ndarray, norms: np.ndarray) -> np.ndarray:
        ...
        single = signs.ndim == 1
        if single:
            signs = signs[np.newaxis, :].astype(np.float32)
        ...
        assert self.S.shape[0] == self.d, (
            f"Projection matrix row count {self.S.shape[0]} must equal d={self.d}"
        )
